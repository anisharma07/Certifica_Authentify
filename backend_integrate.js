...
async function main() {
  ...
  app.post("/signup", async (req, res) => {
    ...
    if (req.body.accountType == "user") {
      ...
      if (req.body.secretKey == Number(Secret)) {
        User = new newUser({
          username: req.body.username,
          password: req.body.password,
          type: "issuer",
  ...
  app.post("/add", async (req, res) => {
    ...
    const certificate = new newCerti({
      _id: req.body.certificateNumber,
      mail: req.body.mail,
      name: req.body.name,
      fathernm: req.body.fatherName,
      rollnm: req.body.rollNumber,
      schoolcd: req.body.schoolCode,
      percentage: req.body.percentage,
    });
  ...
  app.post("/qr-verify", async (req, res) => {
    console.log(req.body);
    const certificate = await newCerti.findOne({
      _id: req.body.certificateNumber,
    });
    const responseData = {
